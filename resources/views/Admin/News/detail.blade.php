...
@section('title', 'Detail Data News')
@section('content')
    ...
    <div class="mx-10 mt-10">
        <div class="relative w-full">
            ...
            <img class="fa-solid fa-comment text-gray-500 dark:text-gray-400 text-4xl mb-4 rounded-md"
                src="{{ asset('img/news/' . $detail->gambar) }}" alt="berita" width="500" />
        </div>
        <div class="relative w-full">
            <label class="block uppercase tracking-wide text-gray-700 text-xl font-bold mb-2" for="grid-password">
                Judul Berita
            </label>
            <input disabled value="{{ $detail->judul_berita }}" name="judul_berita"
        ...
        <div class="w-full bg-gray-200 mt-36">
            <button
                class="mr-6 bg-blue-500 hover:opacity-80 border-transparant text-white font-semibold py-2 px-10 transition rounded shadow" type="button" onclick="return window.location.href = '<?php echo route('datanews'); ?>';">
                Back
            </button>
        </div>
    </div>
